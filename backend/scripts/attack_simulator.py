...
def _risk_colour(score: float) -> str:
    if score < 0.35:  return _GREEN
    if score < 0.70:  return _YELLOW
    return _RED


def _decision_colour(decision: str) -> str:
    d = decision.lower()
    if "allow" in d:   return _GREEN
    if "step" in d:    return _YELLOW
    return _RED


def _bar(score: float, width: int = 20) -> str:
    filled = int(round(score * width))
    c = _risk_colour(score)
    return f"{c}{'█' * filled}{'░' * (width - filled)}{_RESET}"


def _fmt(score: float) -> str:
    return f"{_risk_colour(score)}{score:.3f}{_RESET}"


def _extract_debug(body: Any) -> dict | None:
    if not isinstance(body, dict):
        return None
    dbg = body.get("debug")
    if dbg is None and isinstance(body.get("detail"), dict):
        dbg = body["detail"].get("debug")
    return dbg if isinstance(dbg, dict) else None


def print_outcome(label: str, out: LoginOutcome) -> None:
    dbg    = _extract_debug(out.body)
    sig    = out.signals or {}
    ...
    if isinstance(out.body, dict):
        final = out.body.get("risk")
    if final is None and isinstance(dbg, dict):
        final = dbg.get("final")
    final_f = float(final) if isinstance(final, (int, float)) else None

    # ── decision badge ─────────────────────────────────────────────────────
    decision = out.decision
    dc = _decision_colour(decision)
    badge_map = {
        "allow":              f"{_GREEN}✅  ALLOW{_RESET}",
        "step_up":            f"{_YELLOW}⚠️   STEP-UP (MFA){_RESET}",
        "block":              f"{_RED}🚫  BLOCK{_RESET}",
        "blocked_high_risk":  f"{_RED}🚫  BLOCK{_RESET}",
        "mfa_required":       f"{_YELLOW}⚠️   STEP-UP (MFA){_RESET}",
        "invalid_credentials":f"{_RED}❌  BAD CREDS{_RESET}",
    }
    badge = badge_map.get(decision.lower(),
                          f"{dc}{_BOLD}{decision}{_RESET}")

    # ── header ─────────────────────────────────────────────────────────────
    print(f"\n  {_BOLD}{_WHITE}┌─ [{out.status_code}] {label}{_RESET}")
    print(f"  {_BOLD}{_WHITE}│  ip={out.spoofed_ip}   decision={_RESET}{badge}")

    # ── sub-scores ─────────────────────────────────────────────────────────
    if sig:
        names   = ["velocity", "geo", "device", "behavioral"]
        labels  = ["Velocity ", "Geo      ", "Device   ", "Behavioral"]
        print(f"  {_BOLD}{_WHITE}│{_RESET}")
        print(f"  {_BOLD}{_WHITE}│  Sub-scores:{_RESET}")
        for key, lbl in zip(names, labels):
            v = float(sig.get(key, 0))
            print(f"  {_WHITE}│{_RESET}    {lbl}  {_bar(v)}  {_fmt(v)}")
    else:
        print(f"  {_WHITE}│{_RESET}  {_DIM}(no sub-scores — is DEBUG=True in backend .env?){_RESET}")

    # ── final score bar ────────────────────────────────────────────────────
    if final_f is not None:
        print(f"  {_WHITE}│{_RESET}")
        print(f"  {_WHITE}│  {'Final Risk':10}  {_bar(final_f)}  {_BOLD}{_fmt(final_f)}{_RESET}")

    # ── extra debug info (velocity counts, failed login count) ─────────────
    if isinstance(dbg, dict):
        vc  = dbg.get("velocity_counts")
        flc = dbg.get("failed_login_count")
        loc = (dbg.get("city") or ""), (dbg.get("country") or "")
        loc_str = f"{loc[0]}, {loc[1]}" if any(loc) else ""
        if vc or flc is not None or loc_str:
            bits = []
            if isinstance(vc, dict):
                bits.append(
                    f"velocity counts → user={vc.get('user')}  "
                    f"ip={vc.get('ip')}  user+ip={vc.get('user_ip')}"
                )
            if isinstance(flc, int):
                bits.append(f"failed logins so far: {flc}")
            if loc_str:
                bits.append(f"location: {loc_str}")
            for b in bits:
                print(f"  {_WHITE}│{_RESET}  {_DIM}{b}{_RESET}")

    print(f"  {_WHITE}└{'─'*54}{_RESET}")
...
def scenario_brute_force(base_url: str, email: str, password: str) -> None:
    _section("BRUTE FORCE",
             "15 wrong-password attempts from one IP, then 1 real-creds probe.\n"
             "  Expected: velocity climbs; v= signal lit on the final probe.")
    ip = IP_NP
    tripped = False
    for i in range(15):
        payload = build_payload(email, "wrong-" + uuid.uuid4().hex[:6])
...
def scenario_distributed_brute(base_url: str, email: str, password: str) -> None:
    _section("DISTRIBUTED BRUTE FORCE",
             "Same user, rotated IPs.\n"
             "  Expected: per-IP stays low, but per-USER velocity climbs.")
    for i in range(20):
        ip = f"203.0.113.{random.randint(1, 250)}"   # TEST-NET-3
        payload = build_payload(email, "wrong-" + uuid.uuid4().hex[:6])
        out = post_login(base_url, payload, ip)
...
def scenario_impossible_travel(base_url: str, email: str, password: str) -> None:
    _section("IMPOSSIBLE TRAVEL",
             "Login from US, then 5 s later from AU.\n"
             "  Expected: 2nd login flags geo risk (speed > GEO_MAX_SPEED_KMH).")
    dev = human_device("travel")
    out1 = post_login(base_url, build_payload(email, password, device=dev), IP_US)
    print_outcome("login from US", out1)
    if out1.status_code >= 400:
...
def scenario_new_device(base_url: str, email: str, password: str) -> None:
    _section("NEW DEVICE",
             "Correct password but brand-new fingerprint.\n"
             "  Expected: device signal ~0.5 → likely STEP_UP MFA.")
    payload = build_payload(email, password, device=new_device())
    out = post_login(base_url, payload, IP_NP)
    print_outcome("new device login", out)
...
def scenario_bot_behavior(base_url: str, email: str, password: str) -> None:
    _section("BOT BEHAVIOR",
             "Correct password with bot-like typing + headless flags.\n"
             "  Note: behavioral baseline kicks in after BEHAVIOR_MIN_LOGINS_FOR_BASELINE (5) logins.")
    payload = build_payload(
        email, password,
        device=human_device("bot"),
        behavior=bot_behavior(),
...
def scenario_datacenter_ip(base_url: str, email: str, password: str) -> None:
    _section("DATACENTER IP",
             "Correct password from AWS / Cloudflare / Hetzner IPs.\n"
             "  Expected: geo risk bumps because ASN matches DATACENTER_ASNS.")
    for ip, label in [(IP_AWS, "AWS"), (IP_CF, "Cloudflare"), (IP_DE, "Hetzner")]:
        out = post_login(base_url, build_payload(email, password), ip)
        print_outcome(f"login via {label}", out)
        time.sleep(0.5)
...
def scenario_combined(base_url: str, email: str, password: str) -> None:
    _section("COMBINED ATTACK",
             "New device + foreign datacenter IP + bot typing + headless browser.\n"
             "  Expected: BLOCK or STEP_UP with multiple signals lit.")
    payload = build_payload(
        email, password,
        device=new_device(),
        behavior=bot_behavior(),
...
def main() -> int:
    ...
    print(f"{_BOLD}{_WHITE}  SentinelIQ Attack Simulator{_RESET}")
    print(f"{_BOLD}{_CYAN}{'═' * 60}{_RESET}")
    print(f"  {_DIM}Target : {args.base_url}{_RESET}")
    print(f"  {_DIM}User   : {args.email}{_RESET}")
